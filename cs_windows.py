...
class Module:
    ...
    def on_module_selected(self):
        if not self.loaded:
            ...
            mode_inner = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=4)
            ...
            top_row.pack_start(radio_none,   False, False, 0)
            top_row.pack_start(radio_manual, False, False, 0)
            top_row.pack_start(radio_auto,   False, False, 0)

            hint_label = Gtk.Label(label=_("Use Super+Q to switch between grid and column layout"))
            hint_label.set_halign(Gtk.Align.START)
            hint_label.set_margin_start(32)
            hint_label.get_style_context().add_class("dim-label")

            mode_inner.pack_start(top_row,    False, False, 0)
            mode_inner.pack_start(hint_label, False, False, 0)
            ...
            def update_radios(*args):
                ...
                hint_label.set_visible(is_auto)
                self._tiling_updating = False
