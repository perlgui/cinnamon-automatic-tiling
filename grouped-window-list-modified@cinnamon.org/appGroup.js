...
class AppGroup {
    constructor(params) {
        ...
        // Persistent small order-number badge — bottom-right corner of the icon.
        // Shows the 1-based Super+Number hotkey index at all times.
        this.orderBadge = new St.BoxLayout({
            style_class: 'grouped-window-list-badge grouped-window-list-order-badge',
            important: true,
            x_align: St.Align.MIDDLE,
            y_align: St.Align.MIDDLE,
            show_on_set_parent: false,
        });
        this.orderBadgeLabel = new St.Label({
            style_class: 'grouped-window-list-number-label grouped-window-list-order-label',
            important: true,
            text: ''
        });
        this.orderBadgeLabel.clutter_text.ellipsize = false;
        this.orderBadge.add(this.orderBadgeLabel, {
            x_align: St.Align.START,
            y_align: St.Align.START,
        });
        this.actor.add_child(this.orderBadge);
        this.orderBadge.set_text_direction(St.TextDirection.LTR);
    ...
    allocate(actor, box, flags) {
        ...
        // Set order badge position — bottom-right corner of the icon
        const orderBadgeBox = new Clutter.ActorBox();
        const [oLabelMinWidth, oLabelMinHeight, oLabelNaturalWidth, oLabelNaturalHeight] = this.orderBadgeLabel.get_preferred_size();
        const orderBadgeSize = Math.max(oLabelNaturalWidth, oLabelNaturalHeight);
        const orderBadgeXCenter = this.iconBox.x + this.iconBox.width - 3 * global.ui_scale;
        const orderBadgeYCenter = this.iconBox.y + this.iconBox.height - 3 * global.ui_scale;
        orderBadgeBox.x2 = Math.min(orderBadgeXCenter + Math.floor(orderBadgeSize / 2), box.x2);
        orderBadgeBox.x1 = orderBadgeBox.x2 - orderBadgeSize;
        orderBadgeBox.y2 = Math.min(orderBadgeYCenter + Math.floor(orderBadgeSize / 2), box.y2);
        orderBadgeBox.y1 = orderBadgeBox.y2 - orderBadgeSize;
        const orderLabelPosX = Math.floor((orderBadgeSize - oLabelNaturalWidth) / 2);
        const orderLabelPosY = Math.floor((orderBadgeSize - oLabelNaturalHeight) / 2);
        this.orderBadgeLabel.set_anchor_point(-orderLabelPosX, -orderLabelPosY);
        this.orderBadge.set_size(orderBadgeSize, orderBadgeSize);
        this.orderBadge.allocate(orderBadgeBox, flags);
    ...
    updateOrderLabel(index) {
        if (this.groupState.willUnmount) return;
        this.orderBadgeLabel.text = (index + 1).toString();
