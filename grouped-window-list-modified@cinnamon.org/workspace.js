...
class Workspace {
    ...
    windowAdded(metaWorkspace, metaWindow, app, isFavoriteApp) {
        ...
        // Refresh order labels for all groups since indices may have shifted
        this.appGroups.forEach( (appGroup, i) => {
            appGroup.updateOrderLabel(i);
        });
    }
    ...
    windowRemoved(metaWorkspace, metaWindow) {
        ...
        if (refApp > -1) {
            this.appGroups[refApp].windowRemoved(metaWorkspace, metaWindow, refWindow, (appId, isFavoriteApp) => {
                ...
                // Refresh order labels since indices may have shifted
                this.appGroups.forEach( (appGroup, i) => {
                    appGroup.updateOrderLabel(i);
                });
            });
