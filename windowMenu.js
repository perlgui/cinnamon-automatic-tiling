...
var WindowMenu = class extends PopupMenu.PopupMenu {
    ...
    _buildMenu(window) {
        ...
        // Auto-tile exclusion toggle - only shown when auto-tile is enabled
        try {
            let muffinSettings = imports.gi.Gio.Settings.new('org.cinnamon.muffin');
            if (muffinSettings.get_boolean('auto-tile')) {
                let wmClass = window.get_wm_class() || '';
                let isExcluded = muffinSettings.get_strv('auto-tile-excludelist').includes(wmClass);
                let excludeItem = this.addAction(this, _("E_xclude from Tiling"), () => {
                    let list = muffinSettings.get_strv('auto-tile-excludelist');
                    let cls = window.get_wm_class() || '';
                    if (list.includes(cls))
                        list = list.filter(x => x !== cls);
                    else
                        list = [...list, cls];
                    muffinSettings.set_strv('auto-tile-excludelist', list);
                });
                excludeItem.setOrnament(PopupMenu.OrnamentType.CHECK, isExcluded);
                this.addMenuItem(new PopupMenu.PopupSeparatorMenuItem());
            }
        } catch(e) {}
