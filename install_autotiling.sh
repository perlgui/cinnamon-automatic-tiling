...
WTILING_JS_DEST="/usr/share/cinnamon/js/ui/windowTiling.js"
...
if [[ "${1:-}" == "--uninstall" ]]; then
    ...
    FOUND=0
    for dest in "$SCHEMA_DEST" "$WM_JS_DEST" "$WTILING_JS_DEST" "$WMENU_JS_DEST" "$CS_PY_DEST"; do
    ...
    # windowTiling.js is new (no stock version) - remove it on uninstall
    if [[ -f "$WTILING_JS_DEST" ]]; then
        rm -f "$WTILING_JS_DEST"
        ok "Removed: windowTiling.js"
    fi
...
MISSING=0
for f in "org.cinnamon.muffin.gschema.xml" "windowManager.js" "windowTiling.js" "windowMenu.js" "cs_windows.py"; do
...
# windowTiling.js is new - no backup needed
...
# Step 2b - Install windowTiling.js

section "Step 2b - Installing windowTiling.js"
cp "$SCRIPT_DIR/windowTiling.js" "$WTILING_JS_DEST"
ok "windowTiling.js installed."
...
echo "   ║                                                              ║"
VER_LINE="  Cinnamon version: $CINNAMON_VER"
printf "   ║%-62s║\n" "$VER_LINE"
echo "   ║                                                              ║"
...
echo "   ║    System Settings -> Windows -> Tiling                      ║"
echo "   ║                                                              ║"
echo "   ║  To uninstall:                                               ║"
